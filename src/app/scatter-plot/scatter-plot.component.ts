...
import {SCTLPoint} from './models/sctl-point';
...
export class ScatterPlotComponent implements OnInit, OnChanges, OnDestroy {
      ...
      if (this.data) {
        if (this.filters) {
          this.filters.forEach(color => {
            this.displayData.push(this.data.get(color));
          });
        } else {
  ...
  ngOnChanges(change) {
    if (this.filters && this.data) {
      if (change.filters) {
        this.displayData = [];
        change.filters.currentValue.forEach(color => {
          this.displayData.push(this.data.get(color));
        });
        this.updateData();
      }
  ...
  drawChart(): void {
    ...
    this.voronoi = d3.voronoi()
      .x((d: SCTLPoint) => this.x(d.x))
      .y((d: SCTLPoint) => this.y(d.y))
